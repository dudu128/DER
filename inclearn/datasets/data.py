...
class IncrementalDataset:
    def __init__(
        ...
        print(data_folder)
        self.DA = isDA
        self._ordered = isOrdered
        self.dataset_name = dataset_name
        self.train_csv_file = train_csv_file
        self.thres_csv_file = thres_csv_file
        ...
        self.train_transforms = self.train_dataset.train_transforms
        self.test_transforms = self.train_dataset.test_transforms
        #torchvision or albumentations
        self.transform_type = self.train_dataset.transform_type
        ...
        self.data_others = None
        self.targets_others = None
        self.data_trained = None
        self.targets_trained = None
    ...
    def new_task(self):
        ...
        if self._open == True:
            #把後面的data當others
            min_class, max_class, x_train, y_train, x_test, y_test, x_val, y_val = self._get_cur_step_open_data_set()
        else:
            #只load 0-15類有label的類別
            min_class, max_class, x_train, y_train, x_test, y_test, x_val, y_val = self._get_cur_step_data_for_raw_data()
    ...
    def _get_cur_step_data_for_raw_data(self, ):
        ...
        x_val, y_val = self._select(self.data_val, self.targets_val, low_range=0, high_range=max_class)
        return min_class, max_class, x_train, y_train, x_test, y_test, x_val, y_val
    
    def _get_cur_step_open_data_set(self, ):
        if self._current_task == 0:
            min_class = sum(self.increments[:self._current_task]) + 1
        else:
            min_class = sum(self.increments[:self._current_task])
        if not self._oscil:
            max_class = sum(self.increments[:self._current_task + 1])+1
            total_class = sum(self.increments)+1
        else:
            max_class = sum(self.increments[:self._current_task + 1])
            total_class = sum(self.increments)
        
        x_train_unknown, y_train_unknown = self._select(self.data_train, self.targets_train, low_range=0, high_range=1)
        x_train_others, y_train_others = self._select(self.data_train, self.targets_train, low_range=max_class, high_range=total_class)
        y_train_others = np.zeros(len(x_train_others), dtype=int)
        x_train, y_train = self._select(self.data_train, self.targets_train, low_range=min_class, high_range=max_class)
        
        x_test_others, y_test_others = self._select(self.data_test, self.targets_test, low_range=max_class, high_range=total_class)
        y_test_others = np.zeros(len(x_test_others), dtype=int)
        x_test, y_test = self._select(self.data_test, self.targets_test, low_range=0, high_range=max_class)
        
        x_val_others, y_val_others = self._select(self.data_val, self.targets_val, low_range=max_class, high_range=total_class)
        y_val_others = np.zeros(len(x_val_others), dtype=int)
        x_val, y_val = self._select(self.data_val, self.targets_val, low_range=0, high_range=max_class)
        
        x_train = np.concatenate((x_train_unknown, x_train, x_train_others))
        y_train = np.concatenate((y_train_unknown, y_train, y_train_others))
        x_test = np.concatenate((x_test, x_test_others))
        y_test = np.concatenate((y_test, y_test_others))
        x_val = np.concatenate((x_val, x_val_others))
        y_val = np.concatenate((y_val, y_val_others))
        
        self.data_others = np.concatenate((x_train_unknown, x_train_others))
        self.targets_others = np.concatenate((y_train_unknown, y_train_others))
        
        x_trained, y_trained = self._select(self.data_train, self.targets_train, low_range=0, high_range=max_class)
        self.data_trained = np.concatenate((x_trained, x_train_others))
        self.targets_trained = np.concatenate((y_trained, y_train_others))
    ...
    def _setup_data(self, dataset):
        ...
        train_dataset = dataset(self.data_folder, train=True, DA=self.DA, ordered=self._ordered, csv_file=self.train_csv_file, DA_type=self.DA_type)
        test_dataset = dataset(self.data_folder, train=False, DA=self.DA, ordered=self._ordered, csv_file=self.test_csv_file, DA_type=self.DA_type)
        val_dataset = dataset(self.data_folder, train=False, val=True, DA=self.DA, ordered=self._ordered, csv_file=self.thres_csv_file, DA_type=self.DA_type)
        self.train_dataset = train_dataset
        self.test_dataset = test_dataset
        self.val_dataset = val_dataset
        self.n_tot_cls = self.train_dataset.n_cls  #number of classes in whole dataset

        self._setup_data_for_raw_data(dataset, train_dataset, test_dataset, val_dataset, current_class_idx)
        # !list
    ...
    def _setup_data_for_raw_data(self, dataset, train_dataset, test_dataset, val_dataset, current_class_idx=0):
        ...
        # print(train_dataset.class_order(self.trial_i))
        ...
        elif train_dataset.class_order(self.trial_i) is not None:
            order = train_dataset.class_order(self.trial_i)
...
class DummyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(x, np.ndarray):
            ...
            #         x = cv2.imread(x)
            #         x = x[:, :, ::-1]
            #         self.share_memory[idx] = self.x[idx]
            # else:
            x = cv2.imread(x)
            x = x[:, :, ::-1]
