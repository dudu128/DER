    ...
    def _after_task(self, taski, inc_dataset, val_loader):
        ...
        if (self._cfg["decouple"]['enable'] and taski > 0):
            ...
            if self._cfg["decouple"]["fullset"]:
                if self._cfg["decouple"]["finetune"]:                    
                    train_loader = inc_dataset._get_loader(inc_dataset.data_trained, inc_dataset.targets_trained, mode="train") #完整dataset
                else:
                    train_loader = inc_dataset._get_loader(inc_dataset.data_inc, inc_dataset.targets_inc, mode="train") #一些類別數數sample
            else:
