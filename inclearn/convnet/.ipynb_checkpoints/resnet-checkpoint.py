...
class SCI(nn.Module):
    def __init__(self, in_channels=None):
        ...
        self.conv = nn.Conv2d(in_channels=self.in_channels, out_channels=self.in_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # (B, C, W, H) resize to (B, C, WH) -> X
        x_cl = x.view(1, self.in_channels, -1)
        # Transpose X
        x_cl_t = x_cl.permute(0, 2, 1)
        # W = SoftMax(-XX^T)
        W = nn.functional.softmax(torch.matmul(x_cl*(-1), x_cl_t), dim=-1)
        # Y = WX
        y_cl = torch.matmul(W, x_cl)
        # resize Y back to (B, C, W, H)
        y = y_cl.view(batch_size, self.in_channels, w_size, h_size)
        # Get Phi(Y)
        W_y = self.conv(y)
        # Z = Phi(Y) + X
        z = W_y + x
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        if sci == True:
            ...
            # self.out_dim = 8 * nf * block.expansion * 4
...
def resnet18(pretrained=True, **kwargs):
    ...
    """
    pretrain_model = kwargs.pop('pretrain_model')
    model = ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        if pretrain_model != "":
            pretrained_model = torch.load(pretrain_model)
            pretrained_model = list(pretrained_model.items())
            model_state = model.state_dict()
            count = 0
            for k, v in model_state.items():
                layer_name, weights = pretrained_model[count]
                model_state[k] = weights
                count += 1
...
def resnet50(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load("/home/ldap/chrislin/Incremental_Learning/deepclusterv2_800ep_pretrain.pth.tar"), strict=False)
