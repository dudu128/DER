...
class MemorySize:
    def __init__(self, mode, inc_dataset, total_memory=None, fixed_memory_per_cls=None, openset=False):
        self.mode = mode
    ...
    def update_memory_per_cls_uniform(self, n_classes):
        if "fixed_per_cls" in self.mode:
            ...
            if self.openset == True:
                self.mem_per_cls = [self.total_memory // (n_classes-1) for i in range(n_classes)]
            else:
...
def herding(n_classes, task_size, network, herding_matrix, inc_dataset, shared_data_inc, memory_per_class: list,
    ...
    for class_idx in range(n_classes):
        inputs = inc_dataset.data_inc[inc_dataset.targets_inc == class_idx]
        targets = inc_dataset.targets_inc[inc_dataset.targets_inc == class_idx]

        
        if openset == True:
            if class_idx == 0:
                if inc_dataset._current_task == 1:
                    herding_matrix.append([])
                continue
        
        if class_idx >= n_classes - task_size or inputs.size != herding_matrix[class_idx].size:
            ...
            print(class_idx, len(inc_dataset.data_inc[inc_dataset.targets_inc == class_idx]))
            ...
            if class_idx >= n_classes - task_size:
                ...
                herding_matrix[class_idx] = select_examplars(features, memory_per_class[class_idx])[0]
