...
base_embedded = tsne.fit_transform(reference_output)
print(base_embedded.shape)

# tsne_result_df = pd.DataFrame({'tsne_1': base_embedded[:,0], 'tsne_2': base_embedded[:,1], 'label': y})
tsne_result_df = pd.DataFrame({'tsne_1': base_embedded[:,0], 'tsne_2': base_embedded[:,1]})
fig, ax = plt.subplots(1)
sns.scatterplot(x='tsne_1', y='tsne_2', data=tsne_result_df, ax=ax,s=120)
# sns_plot.figure.savefig("output.png")
plt.savefig(save_path)
# features_embedded = tsne.transform(features)
# features_embedded_oc = tsne.transform(features_oc)
# features_embedded_ow = tsne.transform(features_ow)
# features_embedded_nc = tsne.transform(features_nc)
# features_embedded_nw = tsne.transform(features_nw)
